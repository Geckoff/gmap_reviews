...
var _markersdata = __webpack_require__(95);

var _markersdata2 = _interopRequireDefault(_markersdata);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var gMapsApp = function gMapsApp() {
    var map = void 0,
        popup = void 0,
        markerCluster = void 0,
        markersDataFuncs = void 0;
    ...
    var addEventsToMarker = function addEventsToMarker(marker) {
    ...
    var zoomInToClusterMarker = function zoomInToClusterMarker(geoId) {
    ...
    // generate clusters
    var generateClusters = function generateClusters() {
        if ((typeof markerCluster === 'undefined' ? 'undefined' : (0, _typeof3.default)(markerCluster)) === 'object') {
        ...
        google.maps.event.addListener(markerCluster, 'clusterclick', function (cluster) {
            ...
            cluster.markers_.forEach(function (clusterMarker) {
                var clusterMarkerItself = clusterMarker,
    ...
    var initMap = function initMap() {
        popup = (0, _popup2.default)(google);
        ...
        markersDataFuncs = new _markersdata2.default(map, google);

        // attaching function to event that triggers when save button was pressed
        document.addEventListener('markerUpdate', function (e) {
            if (e.detail.addNewLocation) {
                ...
                setTimeout(function () {
                    if (zoomInToClusterMarker(e.detail.geoId)) {
                        var markerData = markersDataFuncs.storageData[e.detail.geoId],
                            latLng = new google.maps.LatLng(markerData.lat, markerData.lng);

                        new popup.MarkerPopupBuilder([markerData.address, e.detail.geoId, latLng], markersDataFuncs.storageData[e.detail.geoId], false, map).createPopup();
                    }
                }, 100);
            } else {
...
exports.default = function (google) {
    /**
     ...
     */
    var Popup = function Popup(position, content, isMarker) {
    ...
    Popup.prototype = (0, _create2.default)(google.maps.OverlayView.prototype);
    ...
    var inherit = function inherit(child, parent) {
        child.prototype = (0, _create2.default)(parent.prototype);
    ...
    PopupBuilder.prototype.createPopup = function () {
        ...
        this.anchor.querySelector('.close').addEventListener('click', function (e) {
            _this.onRemove();
        });
     ...
     */
    var MarkerPopupBuilder = function MarkerPopupBuilder(geoData, markerData, eventLatLng, map) {
    ...
    MarkerPopupBuilder.prototype.getHtml = function () {
        ...
        if (this.markerData) {
            this.markerData.comments.forEach(function (comment) {
    ...
    MarkerPopupBuilder.prototype.setEvents = function () {
        ...
        this.contentWrapper.querySelector('.save-review').addEventListener('click', function () {
            var review = escapeHtml(_this2.contentWrapper.querySelector('.review').value),
                name = escapeHtml(_this2.contentWrapper.querySelector('.name').value),
                location = escapeHtml(_this2.contentWrapper.querySelector('.location').value),
                date = getDate(),
                geoData = [_this2.address, _this2.geoId, _this2.addresslatLng],
                commentObj = { name: name, location: location, review: review, date: date };

            // checking if all fields filled out      
            if (review && name && location) {
                var markerData = _this2.markerData,
                    addNewLocation = false;

                // for existing Marker just saving comment to Marker's object,
                // for new Location creating new object with Marker Data    
                if (markerData) {
                    _this2.markerData.comments.push(commentObj);
                } else {
                    markerData = {
                        lat: _this2.addresslatLng.lat(),
                        lng: _this2.addresslatLng.lng(),
                ...
                var markerUpdate = new CustomEvent("markerUpdate", {
                    detail: {
                        geoId: _this2.geoId,
                ...
                _this2.onRemove();
                new MarkerPopupBuilder(geoData, markerData, false, _this2.map).createPopup();
            } else {
                _this2.contentWrapper.querySelector('.warning').textContent = "All fileds are required!";
            }
     ...
     * @param {google.maps.Map} map - google maps map obj
     */
    var ClusterPopupBuilder = function ClusterPopupBuilder(geodata, latLng, map) {
    ...
    ClusterPopupBuilder.prototype.getHtml = function () {
        ...
        var _loop = function _loop(geoId) {
            var dataObj = _this3.geodata[geoId].clusterMarkerInfo,
                addressInfo = '<div data-id="' + geoId + '" class="go-to-address cluster-address">' + dataObj.address + '</div>';

            dataObj.comments.forEach(function (comment) {
                ...
                _this3.contentWrapper.appendChild(clusteCommentsBlock);
            });
        };

        for (var geoId in this.geodata) {
            _loop(geoId);
        }
    ...
    ClusterPopupBuilder.prototype.setEvents = function () {
        ...
        this.contentWrapper.addEventListener('click', function (e) {
            if (e.target.classList.contains('go-to-address')) {
                var geoId = e.target.dataset.id,
                    marker = _this4.geodata[geoId].clusterMarkerItself;

                _this4.map.setZoom(21);
                _this4.map.panToWithOffset(marker.position, 0, -220);

                var markerData = _this4.geodata[geoId].clusterMarkerInfo,
                    latLng = new google.maps.LatLng(markerData.lat, markerData.lng);

                new MarkerPopupBuilder([markerData.address, geoId, latLng], markerData, false, _this4.map).createPopup();
            }
        });
    };
    ...
    return {
        ClusterPopupBuilder: ClusterPopupBuilder,
...
* @param {Object} google 
*/
var MarkersData = function MarkersData(map, google) {
    this.map = map;
    this.google = google;
    ...
    this.updateLocalStorage = function () {
        localStorage.data = (0, _stringify2.default)(this.storageData);
    };
    ...
    this.placeMarker = function (geoId, cb) {
        var markerData = this.storageData[geoId],
